...
```{r}
PCA <- gm.prcomp(landmarks.gpa$coords) 
plot(PCA, axis1 = 3, axis2 = 4, main = expression("PC1 and PC2 of "*italic(T.~gracilis)*" shapes"))
```
...
```{r}
plotRefToTarget(PCA$shapes$shapes.comp1$min, PCA$shapes$shapes.comp1$max, method = "vector", mag=1)
```
...
```{r}
plotRefToTarget(PCA$shapes$shapes.comp2$min, PCA$shapes$shapes.comp2$max, method = "vector", mag = 1)
```

### Exploring PC3

#### PC3 Minimum against Maximum

```{r}
plotRefToTarget(PCA$shapes$shapes.comp3$min, PCA$shapes$shapes.comp3$max, method = "vector", mag = 1)
```