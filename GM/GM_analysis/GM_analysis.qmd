...
## 5.1 Linear regression: Change in PC1 over time

```{r}
model_age_pc1 <- geomorph::procD.lm(PCA$x[,1] ~ positions$Age)
```
...
## 5.1 Linear regression: Change in PC2 over time
...
## 5.1 Linear regression: Change in PC3 over time
...
## 5.2 Mean change of shape across beds
...
Ages_PC$sample_names <- as.factor(Ages_PC$sample_names)

PC <- list("Mean" = matrix(nrow = dim(PCA$x)[2], 
                           ncol = length(levels(Ages_PC$sample_names))),
           "SD" = matrix(nrow = dim(PCA$x)[2],
                          ncol = length(levels(Ages_PC$sample_names))))

for (i in 1:dim(PCA$x)[2]) {
  PC[[1]][i,] <- tapply(Ages_PC[,i], Ages_PC$sample_names, mean)
  PC[[2]][i,] <- tapply(Ages_PC[,i], Ages_PC$sample_names, sd)
}
```

```{r}
#| label: plot means and SDs of PC1 per bed

plot(
  x = as.numeric(levels(Ages_PC$sample_names)),
  y = PC[[1]][1,],
  type = "l",
  xlab="Sample number",
  ylab="Mean value of PC1",
  ylim = c(min(PC[[1]][1,] - PC[[2]][1,], na.rm = T),max(PC[[1]][1,] + PC[[2]][1,], na.rm = T))
)

for (i in length(levels(Ages_PC$sample_names))) {
  segments(
    x0 = as.numeric(levels(Ages_PC$sample_names)[i]),
    x1 = as.numeric(levels(Ages_PC$sample_names)[i]),
