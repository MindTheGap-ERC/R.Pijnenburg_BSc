...
height.color.name <- height.colors(Positions$Height)

plot(PCA$x[,1], PCA$x[,2],
     pch = 16,
     bg = height.color.name[Positions$Height],
     cex = scales::rescale(Positions$Height, to = c(0.6, 2)),
     col = height.color.name[Positions$Height],
     xlab = "PC 1",
     ylab = "PC 2",
     main = "PCA of the platform outline of T. gracilis",
     ylim = c(-0.07,0.07)
...
text(x = PCA$x[,1],
     y = PCA$x[,2],
     labels = round(Positions$Height,1),
     pos = 1,
     cex = 0.7
     )
```
